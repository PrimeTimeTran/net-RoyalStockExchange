...
@selectedStocks TABLE (
    ...
    DECLARE
@numStocks INT = ABS(CHECKSUM(NEWID()) % 3) + 3;

WITH RandomizedStocks AS (SELECT TOP(@numStocks) Id, Symbol, Price
                          FROM (SELECT DISTINCT Id, Symbol, Price
                                FROM Stocks
                                WHERE Price >= 10
                                  AND Price <= 500) AS DistinctStocks
                          ORDER BY NEWID())
INSERT
INTO @selectedStocks (StockId, Symbol, Price)
SELECT Id, Symbol, Price
FROM RandomizedStocks;

DECLARE
@stocks NVARCHAR(MAX) = N'[';

    DECLARE
@stockId INT, @symbol NVARCHAR(50), @price DECIMAL(18, 2), @quantity DECIMAL(18, 2);

    DECLARE
stockCursor CURSOR FOR
...
BEGIN
        SET
@quantity = ((ABS(CHECKSUM(NEWID()) % 50) + 1) * 20);

        SET
@stocks += N'{"symbol": "' + @symbol + N'", "quantity": ' + CONVERT(NVARCHAR(MAX), @quantity) + N', "price": ' + CONVERT(NVARCHAR(MAX), @price) + N'},';
...
stockCursor;
    ...
    SET
@stocks = LEFT(@stocks, LEN(@stocks) - 1) + N']';

    DECLARE
@cryptos NVARCHAR(MAX) = N'[
...
@previousValue DECIMAL(18, 2) = (
    ...
    DECLARE
@stocksAndOptionsValue DECIMAL(18, 2) = (
        SELECT SUM(quantity * price)
        FROM OPENJSON(@stocks) WITH (
            quantity DECIMAL(18, 2),
            price DECIMAL(18, 2)
...
@cryptoValue DECIMAL(18, 2) = (
    ...
    DECLARE
@portfolioValue DECIMAL(18, 2) = @stocksAndOptionsValue + @cryptoValue;

    DECLARE
@current NVARCHAR(MAX) = N'{
        "stocks_and_options": {
...
DECLARE
@timeSeries NVARCHAR(MAX) = N'[';

    DECLARE
@changeFactor DECIMAL(6, 2) = 0.01;
    DECLARE
@trendDirection INT = 1;

    DECLARE
@currentValue DECIMAL(18, 2) = @previousValue;

    WHILE
@startTime >= @endTime
BEGIN
        DECLARE
@changePercent DECIMAL(6, 2) = @trendDirection * ((ABS(CHECKSUM(NEWID()) % 200) - 100) / 100.0) * @changeFactor;
        DECLARE
@change DECIMAL(18, 2) = @previousValue * @changePercent;
        SET
@currentValue += @change;
        SET
@previousValue = @currentValue;
        ...
        SET
@timeSeries += N'{"time": "' + REPLACE(CONVERT(NVARCHAR(19), @startTime, 126), 'T', ' ') + N'", "value": ' + CONVERT(NVARCHAR(MAX), @currentValue) + N'},';

        SET
@startTime = DATEADD(MINUTE, @intervalMinutes, @startTime);
END

    SET
@timeSeries = LEFT(@timeSeries, LEN(@timeSeries) - 1) + N']}';
...
SET Valuation = JSON_MODIFY(
        ...
        JSON_QUERY(
                CONCAT(
                        '{"numUnique": ', (SELECT COUNT(*)
                                           FROM OPENJSON(Valuation, '$.cryptocurrencies')), ',',
                        '"totalValue": ', (SELECT CAST(ROUND(SUM(quantity * price), 2) AS DECIMAL(18, 2))
                                           FROM OPENJSON(Valuation, '$.cryptocurrencies')
                                               WITH (
                            symbol NVARCHAR(50),
                        quantity DECIMAL(18, 2),
